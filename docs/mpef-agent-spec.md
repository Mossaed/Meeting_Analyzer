# MPEF Analyzer Agent — Deployment Specification v2.2
...
**v2.2 alignment.** This revision matches analyzer engine v2.2: the Participation table's standalone Q/D/A count columns are gone, folded into the Contributions cell as Q·A·P·R·I·D·T (questions/answers/proposals/risks/info/decisions/actions), and the participant detail view's Questions asked, Answers given, Proposals, Risks and Info sections are now all derived from the same per-utterance classification (`participants[].timeline[].kind`/`.answer`, §2) that produces those counts — so the cell and the drill-down page can never disagree, closing a gap where the cell's uncapped counters and the page's capped `interaction.questions[]` array could show different numbers on a busy transcript. No scoring weight or band changed; the Sample acceptance value is unchanged from v1.6 (**MPI 70.92 · Productive**). Defaults here are identical to the workbook Config sheet and the app `CONFIG` — change one, change all three.
