# MPEF Analyzer Agent — Deployment Specification v2.7
...
```json
{
  ...
  "participants": [{
    ...
    "questions_on_topic": null, "questions_other_item": null, "questions_unclear": null,
    "chat": null, "timeline": null
  }],
  "presenters": [{
  ...
  "interaction": {
    ...
    "questions": null,
    "topic_match_counts": null, "off_topic_status": null,
...
**v2.7** — the report's two question counts now agree: the Participation table's Contributions cell (`participants[].questions`, always uncapped) and the Agenda-drift by member card's per-person Questions column previously disagreed on a busy meeting, because the drift card derived its numbers by filtering `interaction.questions[]`, which is capped at 24 questions meeting-wide (a payload-size limit, unrelated to the actual count). New `participants[].questions_on_topic`/`questions_other_item`/`questions_unclear` and `interaction.topic_match_counts`/`off_topic_status` (§2) carry the same three-way topic-match verdict tallied over **every** question raised, computed once at extraction time — the drift card now reads these instead of re-deriving from the capped array, so its numbers can never fall behind the Contributions cell's on a meeting with more than 24 questions. Also fixes a latent name-collision: the drift card previously matched each row's asker with a prefix test (`sameName`), so two participants whose names prefixed each other (e.g. "Sara" / "Sara Ahmed") would each pick up the other's questions; the new counters are keyed by exact speaker identity, the same key `participants[].questions` itself uses. Legacy Route B JSON predating v2.7 falls back to the original capped derivation. No scoring change; Sample acceptance value unchanged (MPI 70.92).
**v2.6** — fixes "Download report (.html)": the saved file previously carried only markup and the stylesheet, no `<script>`, so every participant's ⋯ drill-down button was inert once opened outside the app. The saved file stays genuinely standalone (still zero `<script>`) — instead, every participant's detail page is pre-rendered into the file at download time and wired up with pure-CSS `:target` navigation: each ⋯ button becomes a plain anchor, detail pages are hidden until they're the URL fragment target, and a sibling selector hides the dashboard while one is shown. `<html lang>`/`dir` now reflect the language active at download time (previously always `en`/ltr). Also fixes the anonymize numbering itself disagreeing between the dashboard and a participant's own detail page (each built its own independent P1/P2… map) — one map, seeded once, is now shared by both. No scoring or extraction change; Sample acceptance value unchanged (MPI 70.92).
