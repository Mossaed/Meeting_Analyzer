# MPEF Analyzer Agent — Deployment Specification v2.0
...
**v2.0 alignment.** This revision matches analyzer engine v2.0: Mode 3 (the deterministic extractor) now runs a full Arabic pattern set — speaker names, decisions, actions, deferrals, risk, feedback, and the agenda-anchoring stemmer — alongside the English one, matched per token so code-switched transcripts are captured by whichever language a given utterance is in; `participants[]` gained `chat` and `timeline` fields (§2) feeding a per-participant detail view, purely additive and never touching scoring; and the report gained metric tooltips and a language toggle (§5), display-layer only — `band`, agenda `status`, and `topic_match` values stay canonical English in the data regardless of display language. No scoring weight or band changed; the Sample acceptance value is unchanged from v1.6 (**MPI 70.92 · Productive**). Defaults here are identical to the workbook Config sheet and the app `CONFIG` — change one, change all three.
...
5. **Participation table** — anonymizable; talk share, contribution mix, and (v2.0) a count of questions asked, decisions stated and actions owned per attendee, plus a detail control opening a full per-participant view (talk share, presenter role, every question/decision/action attributed to them, chat messages, off-agenda contributions, full timeline). Render only attendees with a positively-measured nonzero talk share; an attendee whose share is unmeasurable (`null`) is still listed, but one measured at exactly zero is omitted with a disclosed count ("N attendees present with no recorded speech are not listed") rather than shown at a flat 0% — they still count toward attendance rate, active contributors and speaking balance.
