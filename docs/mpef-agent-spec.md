# MPEF Analyzer Agent — Deployment Specification v2.3
...
**v2.3 alignment.** This revision matches analyzer engine v2.3: input normalization (§1) now recognizes block-format transcripts in either ordering — `Name 0:04` and `02:32:23  Name` — and no longer requires a block header's speaker label to look like a title-case person name, so a recorder export labelling turns "The respective team" or "PMO lead" is captured with its speakers intact instead of folding them into the speech. The looser label is bounded (a few words, no terminal sentence punctuation, not a Decision:/Action: label word) and confined to header lines, so caption-only input keeps `has_speaker_labels: false` and its Not Assessable people metrics. No scoring weight or band changed; the Sample acceptance value is unchanged from v1.6 (**MPI 70.92 · Productive**). Defaults here are identical to the workbook Config sheet and the app `CONFIG` — change one, change all three.
