...
**Applies to:** engine v2.1 · offline edition (`mpef-analyzer-offline.html`) and connected edition (`mpef-analyzer.html`)
...
## 4. Honest-measurement rules built into v2.1
...
Open the file and confirm the dark header reads **engine v2.1** — the filename never changes between versions, so this stamp is the only reliable build check. Then press **Sample → Extract & score locally**: on an unmodified v2.1 build this deterministically renders **MPI 70.92 · Productive** (unchanged from v1.4), six scored dimensions each with a hover/focus tooltip explaining what it measures, a Participation table of 6 (not 8) attendees with a disclosure line for the 2 silent ones and three added count columns (questions/decisions/actions), a `⋯` button on each row opening a full participant detail page, and a flags panel that includes the condensed-transcript and off-agenda-keyword-matching disclosures — the dashboard ends there; it no longer carries its own Decisions, Action items, or Attendee questions cards. Open a participant's detail page and confirm every question and answer shows its surrounding context (the answer given, the resolution status), that decisions stated show the agenda item and the decision text, and that an off-agenda entry names the agenda keywords it was checked against. Click **العربية** at the top-left to switch the whole UI and report to Arabic (RTL, right down to the Sample meeting itself) and back; the choice is not persisted (§10). Full acceptance steps live in the companion checklist.
...
| Symptom | Cause | Fix |
|---|---|---|
| Header doesn't say engine v2.1 | Stale copy (identical filename) | Replace the file; re-share the zip |
