...
**v2.1** — the participant detail page renders the context that was already being extracted but never shown: a question's answer, resolution status and topic match; a decision's preceding lines and basis; and, for off-agenda contributions, the reasoning itself, naming the agenda keywords checked. The dashboard's Decisions, Action items, and Attendee questions cards are retired (that detail now lives on the participant page they duplicated), replaced by a compact **Unattributed items** card that surfaces only what can't be reached from any Participation row — an owner-less action, say. No weight, band, or formula changed; the Sample acceptance value is unchanged at **MPI 70.92**.
...
- **The participant page now shows its evidence, not just its conclusions.** Questions, answers and decisions used to render as bare one-liners even though the extraction already carried a `context` array for each. Questions asked now show the answer received (`context[1]`), the resolution status, and the `topic_match` label; answers given show the question that was being answered, with the participant's own answer as context underneath; decisions stated show the 1-3 preceding transcript lines (`context.slice(0,-1)`) plus the **basis** that qualified them. Off-agenda contributions gained the one thing readers most needed and didn't have: *why* — stated as the rule that actually ran ("an 8-word contribution, matching none of the agenda's keywords (...)"), naming the actual keyword list via a new `quality.agenda_keywords` extraction field (`null` when off-agenda wasn't assessable at all; absent on pre-v2.1 Route B JSON, which falls back to a generic sentence).
